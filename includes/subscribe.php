...
function search_feeds($uid = NULL, $query = NULL, $max = NULL, $ididx = NULL, $withopml = FALSE)
{
    //Check parameters
    if ($uid == NULL) {
        loggit(2, "The user id given is corrupt or blank: [$uid]");
    ...
    if($withopml) {
        $s3url = build_opml_feeds_feed($uid, $max, FALSE, $feeds, FALSE, "search/feedsearch", TRUE, "Feeds search results: [".$query['flat']."]");
        loggit(3, "OPMLURL: $s3url");
        if(is_string($s3url)) {
            $feeds['opmlurl'] = $s3url;
            loggit(3, "OPMLURL: ".$feeds['opmlurl']);
        }
    }
...
//Build an opml outline containing the requested feeds
function build_opml_feeds_feed($uid = NULL, $max = NULL, $archive = FALSE, $feeds = NULL, $nos3 = FALSE, $s3filename = NULL, $returns3url = FALSE, $giventitle = NULL)
{
    //Check parameters
    if ($uid == NULL) {
        loggit(2, "The user id is blank or corrupt: [$uid]");
    ...
    //Set the title
    $outlinetitle = "Subscription list.";
    if(!empty($giventitle)) {
    ...
    foreach ($feeds as $feed) {
        ...
        if (!empty($feed['createdon'])) {
            $opml .= ' created="'.date("D, d M Y H:i", $feed['createdon']).'"';
        }

        if (!empty($feed['link'])) {
            $opml .= " htmlUrl=\"" . htmlspecialchars($feed['link'])."\"";
        }

        if (!empty($feed['url'])) {
            $opml .= " type=\"rss\" xmlUrl=\"" . htmlspecialchars($feed['url'])."\"";
        }
  ...
  ";
    ...
    if ((s3_is_enabled($uid) || sys_s3_is_enabled()) && !$nos3) {
        ...
        if(!empty($s3filename)) {
            ...
            $filename = "feedlisting".time().".opml";
        }
        $arcpath = '';
    ...
    loggit(3, "Built newsfeed opml feed for user: [$username | $uid].");
