...
function build_blog_rss_feed($uid = NULL, $max = NULL, $archive = FALSE, $posts = NULL, $nos3 = FALSE)
{
  ...
  //Get the first and last posts, timewise
  $lastpost = get_first_blog_post($uid);
  $firstpost = $posts[0];
  ...
  if( s3_is_enabled($uid) || sys_s3_is_enabled() ) {
      ...
      <microblog:archive>
          ...
          <microblog:startDay>".date('Y-m-d', $lastpost['createdon'])."</microblog:startDay>
          <microblog:endDay>".date('Y-m-d', $firstpost['createdon'])."</microblog:endDay>
      </microblog:archive>";
  }
  $rss .= "
...
function build_blog_opml_feed($uid = NULL, $max = NULL, $archive = FALSE, $posts = NULL, $nos3 = FALSE)
{
  ...
  $opml .= "
      <head>
        ...
        <dateCreated>".date("D, d M Y H:i:s O", $posts[0]['createdon'])."</dateCreated>
        <dateModified>".date("D, d M Y H:i:s O", $posts[0]['createdon'])."</dateModified>
