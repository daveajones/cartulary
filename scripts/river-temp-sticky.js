<div class="article ${feed.feedId} {{if feed.linkedOutlineType == "
    ...
    <!-- Sticky link -->
    <a class="aUnSticky" href="#" data-id="${id}" data-idx="${index}"
       data-feedid="{{if feed.linkedOutlineType}}{{if feed.linkedOutlineType == 'sopml'}}${feed.linkedOutlineId}{{else}}${feed.feedId}{{/if}}{{else}}${feed.feedId}{{/if}}"><img
        class="icon-unsticky" src="/images/blank.gif" alt=""/></a>

    <!-- Feed level avatar -->
    {{if feed.avatarUrl}}
        {{if feed.linkedOutlineUrl}}<a class="sopmllink" href="#mdlSocialOutlineView" label="View Social Outline"
                                       data-href="${feed.linkedOutlineUrl}">{{/if}}
    ...
    <div class="header">
        <h3>
            {{if permaLink || link}}<a class="articlelink" name="${id}" href="${permaLink || link}" rel="external"
                                       target="_blank">{{/if}}
            {{if title}}
            ${title}
            {{else}}
    ...
    <div class="enclosureview">
        ...
        <a href="${enc.url}" target="_blank">
            ...
            <img class="encobj enclosurepic encpicture2" src="${enc.url}" data-src="${enc.url}" data-type="${enc.type}"
                 data-length="${enc.length}" alt=""/>
            {{else freedomController.v1.river.methods.countEnclosuresOfType(enclosure, 'image') == 3}}
            <img class="encobj enclosurepic encpicture3" src="${enc.url}" data-src="${enc.url}" data-type="${enc.type}"
                 data-length="${enc.length}" alt=""/>
            {{else freedomController.v1.river.methods.countEnclosuresOfType(enclosure, 'image') == 4}}
            <img class="encobj enclosurepic encpicture4" src="${enc.url}" data-src="${enc.url}" data-type="${enc.type}"
                 data-length="${enc.length}" alt=""/>
            {{else freedomController.v1.river.methods.countEnclosuresOfType(enclosure, 'image') >= 5}}
            <img class="encobj enclosurepic encpictures" src="${enc.url}" data-src="${enc.url}" data-type="${enc.type}"
                 data-length="${enc.length}" alt=""/>
            {{else}}
            <img class="encobj enclosurepic encpicture" src="${enc.url}" data-src="${enc.url}" data-type="${enc.type}"
                 data-length="${enc.length}" alt=""/>
            {{/if}}
        </a>
        {{else freedomController.v1.river.methods.isAudio(enc.url, enc.type)}}
        <!-- Audio -->
        <audio class="encobj encaudio" preload="none" controls="controls" data-src="${enc.url}" data-type="${enc.type}" data-length="${enc.length}">
            <source src="${enc.url}" type="audio/mpeg"></source>
        </audio>
        <div class="enclosure ${freedomController.v1.river.methods.getMediaType(enc.type)}">
            <a href="${enc.url}">Download enclosure{{if enc.type && enc.length}} (${enc.type},
                ${freedomController.v1.river.methods.getEnclosureSize(enc.length)}){{/if}}</a>
        </div>
        {{else freedomController.v1.river.methods.isVideo(enc.url, enc.type) && Hidepics == false}}
        <!-- Video -->
        <video class="encobj encvideo" preload="none" controls="controls" data-src="${enc.url}" data-type="${enc.type}" data-length="${enc.length}">
            <source src="${enc.url}" type="${enc.type}"></source>
        </video>
        <div class="enclosure ${freedomController.v1.river.methods.getMediaType(enc.type)}">
            <a href="${enc.url}">Download enclosure{{if enc.type && enc.length}} (${enc.type},
                ${freedomController.v1.river.methods.getEnclosureSize(enc.length)}){{/if}}</a>
        ...
        {{/if}}
        {{/each}}
        {{if freedomController.v1.river.methods.convertYoutube(link) && Hidepics == false}}
        <!-- Embed -->
    ...
    <div class="footer">
            ...
            <a class="jumpTop pull-left" title="Jump back to the top of the page." href="#">
                ...
                <div class="actions">
                    ...
                    <div class="cartform action">
                        <a class="_cartlink"
                           href="<?echo $cartulizecgi?>?title=${encodeURIComponent(title)}&url=${encodeURIComponent(link)}&surl=${encodeURIComponent(feed.feedUrl)}&stitle=${encodeURIComponent(feed.feedTitle)}"
                    ...
                    <div>
                        <a href="<?echo $prefs['linkblog']?>/?description=${encodeURIComponent(title)}&link=${encodeURIComponent(link)}"
                           rel="external nofollow" target="_blank">RT</a>
                    </div>
                    <?} else {?>
        ...
        <div class="subitems">
            ...
            <div class="subitem{{if sub.sticky}} sticky{{/if}}" id="${sub.id}">
                ...
                {{/if}}
                {{/each}}
                {{if sub.permaLink || sub.link}}<a class="articlelink" name="${sub.id}"
                                                   href="${sub.permaLink || sub.link}" rel="external">{{/if}}
                {{if sub.title}}
                ${sub.title}
                {{else}}
