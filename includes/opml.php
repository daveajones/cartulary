...
function buildHtmlFromOpmlRecursive($x = NULL, &$html, $indent = 0, $line = 0, $expansionState = array(), $expand = 1, $expanded = FALSE, &$parents, &$extrahtml)
{
    ...
    foreach ($x->children() as $child) {
        ...
        if( (string)$child->getName() == "outline" ) {
            ...
            if ( $type == "menu" ) {
                $htmlcontent .= "<div class=\"navbar navbar-default navbar-fixed-top navbar-inverse\"><div class=\"container\"><div class=\"row\"><div class=\"nav-collapse\"><ul class=\"nav navbar-nav\"><a class=\"navbar-brand\" href='#'>$nodetext</a>";
            } else
            if ( $type == "tabs" ) {
...
function process_opml_to_html($content = NULL, $title = "", $uid = NULL, $dodisqus = FALSE, $opmlurl = "", $rendertitle = TRUE)
{
    ...
    if( !empty($uid) ) {
        $byline = '<div class="obyline">'.$linktoopml.' by '.get_user_name_from_uid($uid).'</div>';
    } else {
        $authorname = (string)$x->head->ownerName;
        if( !empty($authorname) ) {
            $byline = '<div class="obyline">'.$linktoopml.' by '.$authorname.'</div>';
        }
    }
...
<html>
  <head>
    ...
    <style>
        ...
        li.ou + li.o { margin:8px 8px 8px 0; padding:0px; }
        li.ouwedge {  margin-top:8px;  }
        li.ouwedge + li.o {  margin-top:9px;  }
