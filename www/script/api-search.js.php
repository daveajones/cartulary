...
//things like click bindings and such.
    function _postLoad(query, section, el) {
        ...
        if (section == 'people') {
            freedomController.v1.people.methods.searchPostLoad(query, el);
        }
...
//Take a search query and parse out a section reference from it
    function _parseQuerySection(query) {
        ...
        if (newQuery.indexOf("people:") === 0) {
            return ("people");
        }
        if (newQuery.indexOf("river:") === 0) {
            return ("river");
        }
        if (newQuery.indexOf("microblog:") === 0) {
            return ("microblog");
        }
        if (newQuery.indexOf("articles:") === 0) {
            return ("articles");
        }
        if (newQuery.indexOf("subscribe:") === 0) {
            return ("subscribe");
        }
        if (newQuery.indexOf("editor:") === 0) {
            return ("editor");
        }
