<div class="article ${feed.feedId} {{if feed.linkedOutlineType == "sopml"}}${feed.linkedOutlineId}{{/if}}"
    ...
    <div class="enclosureview">
        ...
        <a href="${enc.url}" target="_blank">
            ...
            <img class="encobj enclosurepic encpicture2 postload" src="${enc.url}" data-src="${enc.url}"
                 data-type="${enc.type}" data-length="${enc.length}" alt=""/>
            {{else freedomController.v1.river.methods.countEnclosuresOfType(enclosure, 'image') == 3}}
            <img class="encobj enclosurepic encpicture3 postload" src="${enc.url}" data-src="${enc.url}"
                 data-type="${enc.type}" data-length="${enc.length}" alt=""/>
            {{else freedomController.v1.river.methods.countEnclosuresOfType(enclosure, 'image') == 4}}
            <img class="encobj enclosurepic encpicture4 postload" src="${enc.url}" data-src="${enc.url}"
                 data-type="${enc.type}" data-length="${enc.length}" alt=""/>
            {{else freedomController.v1.river.methods.countEnclosuresOfType(enclosure, 'image') >= 5}}
            <img class="encobj enclosurepic encpictures postload" src="${enc.url}" data-src="${enc.url}"
                 data-type="${enc.type}" data-length="${enc.length}" alt=""/>
            {{else}}
            <img class="encobj enclosurepic encpicture postload" src="${enc.url}" data-src="${enc.url}"
                 data-type="${enc.type}" data-length="${enc.length}" alt=""/>
            {{/if}}
        </a>
        {{else freedomController.v1.river.methods.isAudio(enc.url, enc.type)}}
        <!-- Audio -->
        <audio class="encobj encaudio" preload="metadata" controls data-src="${enc.url}"
               data-type="${enc.type}" data-length="${enc.length}">
        ...
        <!-- Video -->
        <video class="encobj encvideo" preload="metadata" controls data-src="${enc.url}"
               data-type="${enc.type}" data-length="${enc.length}">
            <source src="${enc.url}" type="${enc.type}"></source>
        </video>
        <div class="enclosure ${freedomController.v1.river.methods.getMediaType(enc.type)}">
        ...
        {{else freedomController.v1.river.methods.convertYoutube(enc.url) && Hidepics == false}}
        <!-- Embed -->
