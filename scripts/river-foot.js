$(document).ready(function () {
    ...
    key('down', function () {	//----- Jump to next feed
        if ( $(freedomController.v1.river.statics.pathToActiveItem).hasClass('cartulized') ) {
            return true;
        }
    ...
    key('shift+t', function () {		//----- Change article title trigger
        $(freedomController.v1.river.statics.pathToStreamItem + ".activeItem button.changetitle").trigger('click');
        return false;
    });
    key('e', function () {		//----- Edit the article link
        $(freedomController.v1.river.statics.pathToStreamItem + ".activeItem div.footer div.actions a.editlink").trigger('click');
        return false;
    });
